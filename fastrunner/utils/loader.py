...
def parse_tests(testcases, debugtalk, project, name=None):
...
def debug_api(api, project, name=None, save=True):
    """debug api
        api :dict or list
        project: int
    """
    try:
        ...
        if isinstance(api, dict):
            ...
            """
            api = [api]
        testcase_list = [parse_tests(api, load_debugtalk(project), project, name=name)]
    ...
    try:
        ...
        if 'case' in test.keys():
            ...
            if test["body"]["method"] == "config":
                ...
                case_step = models.Case.objects.get(id=test['id'])
